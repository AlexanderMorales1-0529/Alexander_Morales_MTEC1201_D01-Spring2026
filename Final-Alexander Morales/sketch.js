...
let victoryParticles = []; // particles for victory screen
...
const IDLE_FRAME_HOLD = 20;          // lower number makes Mateo idle animation faster
...
class Player {
  ...
  draw() {
    ...
    if (this.dead) {
      ...
      img = playIdleGif(img, frameCount);
    }
...
class Boss {
  ...
  update() {
    ...
    if (this.dead) {
      ...
      // After death animation, show victory screen.
...
function drawWinScreen() {
  // Initialize victory particles if not already created.
  ...
  for (let i = victoryParticles.length - 1; i >= 0; i--) {
    ...
    if (p.y < artifactY - 50) {
      p.x = mateoX + random(-25, 25);
      p.y = artifactY + random(5, 25);
      p.vx = random(-0.7, 0.7);
      p.vy = random(-1.2, -0.2);
      p.sparkleColor = random() > 0.5 ? "cyan" : "gold";
    }
  }
...
function createVictoryParticles() {
  for (let i = 0; i < 30; i++) {
    victoryParticles.push({
      ...
      vy: random(-1.2, -0.2),
      size: random(2, 4),
      sparkleColor: random() > 0.5 ? "cyan" : "gold"
    });
  }
}
...
function playIdleGif(img, timer) {
  // If p5 cannot control this GIF, just return the image.
  if (!img || typeof img.numFrames !== "function" || typeof img.setFrame !== "function") return img;

  // Loop idle animation with frame hold value.
  let frames = img.numFrames();
  let frame = floor((timer / IDLE_FRAME_HOLD)) % frames;
